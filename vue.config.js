...
module.exports = {
  ...
  devServer: {
    ...
    proxy: {
      ...
      '/python_api/api_visual_linechart/':{
        target: 'http://120.26.177.17:5500', // 接口的域名
        // secure: false,  // 如果是https接口，需要配置这个参数
        changeOrigin: true, // 如果接口跨域，需要进行这个参数配置
        pathRewrite: {
          '^/python_api/api_visual_linechart/': '/python_api/api_visual_linechart/'
         }
      },
      '/python_api/api_visual_linechart_rules/':{
        target: 'http://120.26.177.17:5500', // 接口的域名
        // secure: false,  // 如果是https接口，需要配置这个参数
        changeOrigin: true, // 如果接口跨域，需要进行这个参数配置
        pathRewrite: {
  ...
  chainWebpack(config) {
    ...
    config
      .when(process.env.NODE_ENV !== 'development' && process.env.NODE_ENV !== 'local',
        config => {
          config
